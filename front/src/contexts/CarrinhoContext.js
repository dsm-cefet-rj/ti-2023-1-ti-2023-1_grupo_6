...
const CarrinhoContextProvider = ({ children }) => {
        ...
        if (novoCarrinho.produtos[i].quantidade) {
          console.log(
            "Quantidade do produto " +
              (i + 1) +
              ": " +
              parseInt(parseInt(novoCarrinho.produtos[i].quantidade))
          );
          console.log(
            "Preço do produto " +
              (i + 1) +
              ": " +
              novoCarrinho.produtos[i].valor
          );
          somadoTotal +=
            parseFloat(novoCarrinho.produtos[i].valor.replace(",", ".")) *
            novoCarrinho.produtos[i].quantidade;
  ...
  const removerProdutoCarrinho = (id) => {
    ...
    const totalValor = novoCarrinho.produtos.reduce(
      (total, p) =>
        total + parseFloat(p.valor.replace(",", ".")) * p.quantidade,
      0
    );
